...
2. **OpenAI-compatible** — ``LLM_BASE_URL`` + ``LLM_API_KEY`` + ``LLM_MODEL``;
   works with GitHub Models (https://models.github.ai/inference), OpenAI,
   or any compatible gateway.
3. **Mock** — deterministic offline responses (also forced by ``LLM_MODE=mock``).
...
class LLMAdapter:
    ...
    def generate_reasoning(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"summary": summary, "mode": self.provider, "model": self.azure_deployment if self.provider == "azure" else self.model}
